...
import sase.user.sensors.SensorActivityChangeCondition;
import sase.user.stocks.condition.StockDeltaOrderingCondition;

import java.util.ArrayList;
import java.util.List;

public class IterativeSensorActivityChangeConditionSpecification extends ConditionSpecification {
    private final String precedingEventName;
    private final String iterativeEventName;
    private final String succeedingEventName;
    public IterativeSensorActivityChangeConditionSpecification(String precedingEventName,
    ...
    public List<AtomicCondition> createConditions() {
        ...
        result.add(new FirstValueExternalCondition(new SensorActivityChangeCondition(precedingType, iterativeType), false));
                ...
                for (int i = 1; i < primitiveEvents.size(); i++) {
                    ...
                    SingleEventCondition condition = new SingleEventCondition(iterativeType) {
                        ...
                        protected boolean verifySingleEvent(Event event) {
                            ...
                            SensorActivityChangeCondition sensorActivityCond = new SensorActivityChangeCondition(iterativeType, iterativeType);
                            return sensorActivityCond.verifySpecificCondition(secondToLastEvent, lastEvent);
                        }
