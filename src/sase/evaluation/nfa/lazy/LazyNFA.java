...
public abstract class LazyNFA extends NFA {
	...
	public LazyNFA(Pattern pattern) {
		...
		//NOTE: will not work in the sase.multi-sase.pattern setting
	...
	private List<Event> prepareAggregatedBufferedEvents(LazyInstance instance, LazyTransition transition,
		...
		//Has to be fixed when iterations and the end of the sase.pattern will be evaluated
	...
	private List<Event> getRelevantBufferedEvents(LazyInstance instance, 
		...
		switch(action) {
			...
			case STORE:
			default:
				throw new RuntimeException(String.format("Illegal edge action for buffer sase.evaluation: %s", action));
		}
	}
